...
export const generateAllData = (): void => {
 ...
 * }>}
 */
export const generateQuestions = (surveys: Array<{
    name: string;
    id: string;
    createdAt: number;
}>): Array<{
 ...
 * }}
 */
export const generateQuestion = (surveyId: string): {
    id: string;
    surveyId: string;
    title: string;
    type: 'single-line-answer' | 'multi-line-answer' | 'multiple-select';
 ...
 */
const pickRandomFromArray = (array: any[], nbElementsMin: number, nbElementsMax?: number): any[] => {
    if (!nbElementsMax) {
